...
extract_snvs = function(file){
    ...
    header = as.vector(unlist(head.table(files[1], sep = "\n", comment.char = "@", nrow = n, skip = n-1)))
    header[1] = "X.CHROM"
    
    raw_exome = suppressWarnings(read_delim(file, delim = "\t", comment = "#", col_names = header, progress = F))
    names(raw_exome)[grep("\\.fa", header)] = "FA"
    names(raw_exome)[grep("\\.mo", header)] = "MO"
    filtered = raw_exome[which(grepl(";DP=[2-9][0-9];|;DP=[1-9][0-9][0-9]", raw_exome$INFO) & raw_exome$QUAL > 50 & grep("^AC=[1-9];|;AC=[1-9];", raw_exome$INFO) & raw_exome$X.CHROM %in% c(1:22, "X", "Y", paste("chr", c(1:22, "X", "Y", "M"), sep = ""), "MT")),]
    rm(raw_exome)
...
if(length(genomes) > batch_size){
    tmp = mclapply(genomes[start[batch]:end[batch]], extract_snvs, mc.cores = 20)
}else{
    tmp = mclapply(genomes, extract_snvs, mc.cores = 20)
}
