...
Dependencies: GATK3.8, java8, genome reference
...
Dependencies: R, tidyverse, parallel, exomes: exomes/batch_private_list.R, genomes: genomes/batch_private_list.R, gather_private_list_batches.R
...
Dependencies: SNPEFF, java, CADDv1.4, ExAC, dbSNP

**NOTES:** CADDv1.4, ExAC, dbSNP VCFs will need to be downloaded/present (these are currently located here: `/net/eichler/vol27/projects/autism_inheritance/nobackups/db/`). Make sure you have alread cached the genome data necessary to run SNPEFF (refer to SNPEFF documentation if using for the first time).
...
Dependencies: BEDTools, centromere and gaps, LCRs, recent repeats, PARs (these data are located here: `/net/eichler/vol27/projects/autism_inheritance/nobackups/reference/`)
